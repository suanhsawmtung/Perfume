...
export function ReceiptPDFButton({ order }: { order: OrderType }) {
  const handleDownloadPDF = async () => {
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Receipt #: ${order.code}`, pageWidth - 20, 35, {
      align: "right",
    });
    doc.text(`Date: ${formatDate(order.createdAt)}`, pageWidth - 20, 42, {
      align: "right",
    });
    ...
    doc.text(formatPrice(order.totalPrice), pageWidth - 25, yPos, {
      align: "right",
    });
    yPos += 8;
    ...
    doc.text(formatPrice(order.totalPaidAmount), pageWidth - 25, yPos, {
      align: "right",
    });
    ...
    if (order.totalRefundAmount > 0) {
      ...
      doc.text(
        formatPrice(order.totalPaidAmount - order.totalRefundAmount),
        pageWidth - 25,
        yPos,
        { align: "right" },
      );
