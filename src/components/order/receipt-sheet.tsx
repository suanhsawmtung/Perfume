  ...
  return (
    <div className="flex items-start justify-between">
      ...
      <div className="text-right">
        ...
        <p className="text-muted-foreground mt-1 text-sm">{order.code}</p>
      </div>
    </div>
  );
}

function ReceiptDetailItem({
  label,
  value,
  fullWidth = false,
}: ReceiptDetailItemProps) {
  return (
  ...
  return (
    <div className="mt-6 grid grid-cols-2 gap-4 text-sm">
      ...
      <ReceiptDetailItem
        ...
        value={`Bank Transfer (${order.paymentStatus})`}
        fullWidth
      />
    </div>
  );
...
function ReceiptItems({ order }: ReceiptSectionProps) {
  return (
    <div className="space-y-4">
      <div className="text-muted-foreground grid grid-cols-12 gap-2 text-xs font-medium uppercase">
